# -*- coding: utf-8 -*-
"""evidence — the gate that must pass BEFORE a review sheet is written.

Lifted (not rewritten) from
``SanskritLexicography/RussianTranslation/src/review_evidence_preflight.py``
(H1887, 29-07-2026) into the shared emitter package, so that every generator in
every repo is gated by the same code instead of one repo's copy. The donor now
re-exports this module.

## Why this exists

On 29-07-2026 MG opened
``sanskritlexicography-pwg-compound-differs_stratified200_review.html`` and asked
why he was being made to vote at all. The measured answer: **191 of its 200 cards
already had a machine verdict, a named rule and cited evidence** — computed by
``src/pilot/adjudicate_compound_differs.py`` (H1681) from the SAME two input
files the sheet itself reads, sharing **4,246 of 4,246 row ids** with the sheet's
frame. The sheet rendered none of it. A further **69 of 200** were not
disagreements at all.

Nothing in the stack was responsible for noticing. The review-sheet standard
(V1–V8 + H1808) is entirely about PRESENTATION — type scale, anatomy colouring,
tooltips, ids, note height — so a sheet can be 100 % standard-compliant and still
ask a human to re-derive, by eye, a conclusion the repo already holds on disk.
The prose rule that should have caught it ("check prior art before building") is
exactly the kind of rule that does not fire, because firing depends on the author
remembering to look.

So this module makes the machine look. It is deterministic, cheap (a key-overlap
scan over the repo's own tabular artifacts) and it BLOCKS: a generator that
cannot show an evidence manifest does not get to write HTML.

## What it checks

1. **Prior-art overlap (the H1628 defect).** Given the sheet's row ids, scan the
   repo for any OTHER committed artifact keyed on the same ids. Any artifact
   overlapping above ``overlap_threshold`` (default 0.5) that the generator did
   not declare as joined is a BLOCK: the sheet is about to ask a human something
   the repo may already answer.
2. **Evidence floor.** Every card must carry at least ``min_evidence_fields``
   joined evidence fields beyond the bare question, or an explicit
   ``omitted_because`` reason naming the source and why it is unavailable. Silent
   absence is a BLOCK; a stated absence is allowed and recorded.
3. **Script purity.** No human-facing string may mix Cyrillic with IAST
   diacritics inside one word (the ``Пāṇini`` defect).
4. **No raw transliteration-scheme leakage.** SLP1 must not appear in
   human-facing text. Humans read IAST; SLP1 belongs in ids and machine columns.
5. **Citation linkability.** A reference with a known URL template (Pāṇini sūtra,
   Cologne entry, DCS locus) is rendered as a link, not a bare string.
6. **Structural validity of cited references.** A citation that cannot exist is
   never shown: an "Aṣṭādhyāyī sūtra" with adhyāya > 8 or pāda > 4 is not a sūtra
   reference (measured 29-07-2026: 625 / 1,270 = 49.2 % of the references the
   compound sheet was about to print were structurally impossible — Ṛgveda
   citations swept into a ``panini_sutras`` column upstream).

Two asymmetries are deliberate and must survive any future edit:

* a conceptual omission (``declare_omitted``) can never silence a FOUND artifact
  — only ``declare_omitted_path`` can, because a real file on disk must not be
  waved away by a vaguely-worded note;
* the SLP1 detector stays **silent** on the undecidable all-lowercase case rather
  than guessing (``agni + deva`` is byte-identical in SLP1 and IAST).

## Usage

    from csl_pyutil.evidence import EvidenceManifest, preflight

    man = EvidenceManifest(sheet_id="...", row_ids=[...], repo_root=REPO)
    man.declare_joined("research/adjudication.tsv", fields=["verdict", "rule"])
    man.declare_omitted("DCS attested sentence",
                        because="no per-compound sentence map exists")
    for card in cards:
        man.add_card(card_id, evidence_fields=[...], omitted=[...])

    render_review_sheet(items, config, screening=..., manifest=man)  # V9

``render_review_sheet(manifest=...)`` runs :func:`preflight` against the finished
document and raises :class:`PreflightError` before returning any HTML.
"""
import io
import json
import os
...
__all__ = [
    ...
    "find_mixed_script", "find_slp1", "selftest",
]
...
class PreflightError(Exception):
    ...
    def __init__(self, findings):
        ...
        super(PreflightError, self).__init__(
            ...
            % (len(self.findings), '\n'.join('  - ' + f for f in self.findings)))


class PreflightWarning(UserWarning):
    """Emitted when a sheet is rendered with no evidence manifest at all.

    A warning, not an error, purely as a migration ramp for the generators that
    predate V9 — escalate it with ``-W error::csl_pyutil.evidence.PreflightWarning``
    (or ``warnings.simplefilter``) to get the eventual 1.0.0 behaviour today.
    """
...
class EvidenceManifest(object):
    ...
    ``repo_root`` defaults to the current working directory — the donor module
    could default to its own parent because it lived inside the one repo it
    scanned; a shared package cannot, and guessing a repo root from the package's
    install location would scan site-packages.
    """

    def __init__(self, sheet_id, row_ids, repo_root=None,
                 min_evidence_fields=2):
        self.sheet_id = sheet_id
        self.row_ids = [str(r) for r in row_ids]
        self.repo_root = os.getcwd() if repo_root is None else repo_root
...
def preflight(manifest, html, allow_slp1_tokens=(), overlap_threshold=0.5,
    ...
    if starved:
        f.append('EVIDENCE FLOOR: %d card(s) carry fewer than %d joined evidence '
                 ...
                 % (len(starved), manifest.min_evidence_fields, ', '.join(sorted(starved)[:5])))
...
# ---------------------------------------------------------------- selftest
def selftest():
    """The donor's own selftest, kept executable so the shim's `--selftest` CLI
    (and any repo that wired it into CI) keeps working after the lift. The same
    assertions run under pytest in tests/test_evidence.py."""
    ...
    return ('selftest OK — %d groups: sutra validity, reference splitting, sutra hrefs, '
